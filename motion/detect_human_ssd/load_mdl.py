...
def load_mdl(model_path=None, label_path=None):
    ...
    net = create_mobilenetv2_ssd_lite(
        len(class_names), is_test=True, device=torch.device("cpu")
    )
    net.load(model_path)

    predictor = create_mobilenetv2_ssd_lite_predictor(
        net, candidate_size=200, device=torch.device("cpu"), class_names=class_names
    )
    return predictor
